...
public class Summative {
	...
	public static boolean inputCheck(String playerInput) {
	...
	//parameters w and h will only be used if game is text-based. Board, frame and canvas will only be used if graphics are used 
	 public static void print(char[][] board, JFrame frame, Canvas canvas, int lastPlay, int w, int h) {
		...
		//commented section was used for text-based game. it printed out the array 
		...
		//to play text-based game, comment out section below, and uncomment the section above.
	 ...
	 public static char[][] place(char[][] board, int w, int h, int bottomR, int p){
			...
			if(p == 1) {//distingishes between the player that is playing
			...
			while(turnNotDone) {
				...
				//with every failed attempt to find an empty spot in the row, the counter increses
				counter++;
				
				//if column is full, the player is prompted to input another value
				if(counter == w) {
	 ...
	 public static boolean playerCheck(char[][] b, int w, int h, int p) {
		...
		public static boolean verticalCheck(char[][] board, int w, int h, int p) {
	        ...
	        while(result){ 
	            ...
	            for(int i = 0; w > i; i += 1){
	                ...
	                for(int j = 0; h > j; j += 1){
	                	...
	                	if(p == 1) {//distinguishes between player playing
	                		  ...
	                		  if(board[j][i] == 'X'){ //if it finds an X, add 1 to counter
	                		  ...
	                		  if(board[j][i] == 'O'){ //if it finds an X, add 1 to counter
	            ...
	            break; //break is used in case no winning combinations detected 
	        }
 ...
 * 	- Code was used to help understand the logic associated with printing turns and checking for victories
 * 
 */
